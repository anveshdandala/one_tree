...
import { Plus, X, Pencil, Trash2, CheckCircle2, AlertCircle, Sparkles } from "lucide-react";
import {
  ReactFlow,
  Background,
  BackgroundVariant,
    ...
    nodes: [
      {
        ...
        description: "Learn semantic HTML, CSS box model, flexbox, and grid layout.",
      ...
      },
      {
        ...
        description: "Understand ES6+ features, async programming, and DOM manipulation.",
      ...
      },
      {
        ...
        description: "Learn state management, hooks, components, and virtual DOM.",
  ...
  },
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        description: "Master NumPy, Pandas, and Matplotlib for data processing.",
      ...
      },
      {
        ...
        description: "Train regressions, decision trees, and SVMs using Scikit-Learn.",
...
const CustomNode = ({ data }) => {
  const { title, description, isCompleted, color, branchTitle, onToggleComplete } = data;
...
// ----------------------------------------------------------------------
export default function HomePageContent({ branches: initialPropsBranches = [] }) {
  // --- Core State ---
  ...
  useEffect(() => {
    ...
    if (localBranches && localEdges) {
      ...
      const defaultBranches = initialPropsBranches.length > 0 ? initialPropsBranches : INITIAL_BRANCHES;
  ...
  const getNewNodePosition = useCallback((branchId) => {
    const branch = branches.find((b) => b.id === branchId);
    if (branch && branch.nodes && branch.nodes.length > 0) {
      const rightmost = [...branch.nodes].sort((a, b) => b.positionX - a.positionX)[0];
      return {
  ...
  const updateNodeField = useCallback((nodeId, field, value) => {
    ...
    }).catch((err) => console.log(`Background API update ${field}:`, err.message));
  }, []);
  ...
  const deleteNode = useCallback((nodeId) => {
    ...
    setEdges((currentEdges) =>
      currentEdges.filter((edge) => edge.sourceId !== nodeId && edge.targetId !== nodeId)
    );
    ...
    fetch(`/api/nodes/${nodeId}`, {
      method: "DELETE",
    }).catch((err) => console.log("Background API delete node:", err.message));
  }, [selectedNodeId]);
  ...
  const onNodeDragStop = useCallback((event, dragNode) => {
    ...
    }).catch((err) => console.log("Background API save coordinates:", err.message));
  }, []);
  ...
  const onConnect = useCallback((connection) => {
    ...
    }).catch((err) => console.log("Background API connect edge:", err.message));
  }, [edges]);
  ...
  const handleBranchSubmit = (e) => {
    ...
    if (editingBranchId) {
      ...
      }).catch((err) => console.log("Background API update branch:", err.message));
    } else {
      ...
      }).catch((err) => console.log("Background API create branch:", err.message));
    }
  ...
  const deleteBranch = useCallback((branchId) => {
    ...
    setEdges((prevEdges) =>
      prevEdges.filter((e) => !nodeIds.includes(e.sourceId) && !nodeIds.includes(e.targetId))
    );
    ...
    fetch(`/api/branches/${branchId}`, {
      method: "DELETE",
    }).catch((err) => console.log("Background API delete branch:", err.message));
  }, [branches, selectedNodeId]);
  ...
  const handleNodeSubmit = (e) => {
    ...
    setBranches((prev) =>
      prev.map((b) =>
        b.id === selectedBranchForNode ? { ...b, nodes: [...(b.nodes || []), newNode] } : b
      )
    );
    ...
    return (
      <div className="flex h-[calc(100vh-73px)] w-full items-center justify-center bg-canvas-bg font-sans text-canvas-muted">
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-xs uppercase tracking-widest font-bold">Configuring Canvas...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-[calc(100vh-73px)] w-full overflow-hidden bg-canvas-bg font-sans text-canvas-text transition-colors duration-200">
      ...
      <div className="w-80 border-r border-canvas-border bg-canvas-card flex flex-col h-full overflow-hidden shrink-0">
        <div className="flex items-center justify-between border-b border-canvas-border px-5 py-4 shrink-0">
          ...
          <button
            onClick={() => {
              setBranchFormValues({ title: "", objective: "", color: "#6366f1" });
              setEditingBranchId(null);
              setIsBranchModalOpen(true);
            }}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {branches.length === 0 ? (
            <div className="text-center py-12 px-4 border border-dashed border-canvas-border rounded-xl">
              ...
              <p className="text-xs text-canvas-muted font-medium">No branches created yet.</p>
              <button
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <h4 className="text-xs font-extrabold text-canvas-text truncate">{branch.title}</h4>
                    {branch.objective && (
                      <p className="text-[10px] text-canvas-muted leading-tight mt-0.5 truncate">{branch.objective}</p>
                    )}
                  </div>
        ...
        <div className="border-b border-canvas-border bg-canvas-card/50 px-6 py-3 shrink-0 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-2 font-mono text-[9px] text-canvas-muted uppercase">
              ...
              <span className="text-canvas-border-strong">•</span>
              <span className="text-emerald-500 font-bold">{completedCount}/{nodeCount} Completed</span>
            </div>
          </div>
              ...
              <div className="space-y-1.5">
                {selectedNodeEdges.length === 0 ? (
                  <p className="text-[10px] text-canvas-dim italic leading-normal">
                    No pathways connected. Drag handles on the graph canvas to link steps.
                  </p>
                ) : (
                    ...
                    return (
                      ...
                      >
                        <span className="text-[10px] text-canvas-text truncate max-w-[190px]">
                          {isSource ? (
                            <span>Leads to: <strong>{partnerNode.title}</strong></span>
                          ) : (
                            <span>Prerequisite: <strong>{partnerNode.title}</strong></span>
                          )}
                        </span>
                        <button
            ...
            <form onSubmit={handleBranchSubmit} className="mt-5 space-y-4">
              <label className="block space-y-1">
                ...
                <input
                  ...
                  onChange={(e) => setBranchFormValues({ ...branchFormValues, title: e.target.value })}
                  maxLength={60}
              ...
              <label className="block space-y-1">
                ...
                <input
                  ...
                  onChange={(e) => setBranchFormValues({ ...branchFormValues, objective: e.target.value })}
                  maxLength={120}
                  className="w-full border border-canvas-border bg-canvas-secondary/50 px-3 py-2.5 text-xs text-canvas-text focus:border-canvas-border-strong focus:outline-none rounded-lg"
                />
              </label>
                ...
                <div className="grid grid-cols-8 gap-2">
                  {COLOR_PALETTE.map((color) => (
                    <button
                      key={color.value}
                      type="button"
                      onClick={() => setBranchFormValues({ ...branchFormValues, color: color.value })}
            ...
            <form onSubmit={handleNodeSubmit} className="mt-5 space-y-4">
              <label className="block space-y-1">
                ...
                <input
                  ...
                  onChange={(e) => setNodeFormValues({ ...nodeFormValues, title: e.target.value })}
                  maxLength={60}
                  className="w-full border border-canvas-border bg-canvas-secondary/50 px-3 py-2.5 text-xs text-canvas-text focus:border-canvas-border-strong focus:outline-none rounded-lg"
                  autoFocus
                />
              ...
              <label className="block space-y-1">
                ...
                <input
                  ...
                  onChange={(e) => setNodeFormValues({ ...nodeFormValues, description: e.target.value })}
                  maxLength={120}
                  className="w-full border border-canvas-border bg-canvas-secondary/50 px-3 py-2.5 text-xs text-canvas-text focus:border-canvas-border-strong focus:outline-none rounded-lg"
                />
              </label>
